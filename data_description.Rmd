...
```{r}
data.frame(hp=seq(0, 100,len=100))
```
...
The variable "imp_prodvarde", is a boolean variables denoting whether the production value was imputed or not. Imputation is applied on companies that were too small to be sampled, and for companies that did not enter values in the survey.
